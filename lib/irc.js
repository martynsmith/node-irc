    ...
    self.addListener("raw", function (message) { // {{{
        switch ( message.command ) {
            ...
            case "TOPIC":
                ...
                break;
            case "rpl_whoisuser":
                self.emit('whoisuser', message.args[1], message.args[2], message.args[3], message.args[5]);
                break;
            case "rpl_whoischannels":
                var whoischannels = message.args[2].trim().split(' ');
                var channum = 0;
                while (channum < whoischannels.length) {
                    if (whoischannels[channum].charAt(0) === '+' || whoischannels[channum].charAt(0) === '@' || whoischannels[channum].charAt(0) === '&' || whoischannels[channum].charAt(0) === '%') {
                        whoischannels[channum] = whoischannels[channum].slice(1);
                    }
                    channum ++;                
                }
                self.emit('whoischannels', message.args[1], whoischannels);
                break;
            case "330":
                self.emit('whoisauthed', message.args[1], message.args[2]);
                break;
            case "rpl_endofwhois":
                self.emit('endofwhois', message.args[1]);
                break;
            case "err_nosuchnick":
                self.emit('nosuchnick', message.args[1]);
                break;
            ...
            case "QUIT":
                ...
                break;
            default:
                if ( message.commandType == 'error' && message.rawCommand !== '401' ) {
...
Client.prototype.whois = function(nickname, callback) { // {{{
    var whois = new Object();
    var whoisuser = this.on('whoisuser', function(whonickname,username,host,realname) {
        if (nickname == whonickname) {
            whois.nickname = whonickname;
            whois.username = username;
            whois.host = host;
            whois.realname = realname;
            var whoisuser = undefined;
        }
    });
    var whoischannels = this.on('whoischannels', function(whonickname, channels) {
        if (nickname == whonickname) {
            whois.channels = channels;
            var whoischannels = undefined
        }
    });
    var whoisauthed = this.on('whoisauthed', function(whonickname, authname) {
        if (nickname == whonickname) {
            whois.auth = authname;
            var whoisauthed = undefined;
        }
    });
    var nosuchnick = this.on('nosuchnick', function(whonickname) {
        if (nickname == whonickname) {
            var endofwhois = undefined;
            var whoisauthed = undefined;
            var whoischannels = undefined;
            var whoisuser = undefined;
            var nosuchnick = undefined;
            callback(null);
        }
    });
    var endofwhois = this.on('endofwhois', function(whonickname) {
        if (nickname == whonickname) {
            if (whois.auth === undefined) {
                whois.auth = null;
            }
            if (whois.channels === undefined) {
                whois.channels = null;
            }
            Object.freeze(whois);
