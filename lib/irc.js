        ...
        switch ( message.command ) {
            ...
            case "005":
                message.args.forEach(function(arg) {
                    var match1, match2;
                    if ( match = arg.match(/PREFIX=\(([a-z]?)\)([!-*])/) ) {
                        match1 = match[1].split('');
                        match2 = match[2].split('');
                        while ( match1.length ) {
                            self.modeForPrefix[match2[0]] = match1[0];
                            self.prefixForMode[match1.shift()] = match2.shift();
                        }
                    }
                });
                break;
            ...
            case "PING":
                ...
                break;
            case "PONG":
                break; // Don't break if PING is sent by a client
            case "NOTICE":
...
exports.parseMessage = function (rawLine, stripColors) { // {{{
    var message = {};
    var match;
...
var parseMessage = exports.parseMessage;
