    ...
    self.addListener('raw', function(message) {
        ...
        switch (message.command) {
            ...
            case 'err_nicknameinuse':
                if (typeof (self.opt.nickMod) == 'undefined' || typeof self.opt.nickMod == 'number') {
                    self.opt.nickMod = (!self.opt.nickMod) ? 1 : self.opt.nickMod++;
                    self.nick = self.opt.nick + self.opt.nickMod;
                } else if (typeof (self.opt.nickMod) == 'function') {
                    self.nick = self.opt.nickMod(self.nick);
                } else self.nick = self.opt.nickMod;

                self.send('NICK', self.nick);
                self.emit('nickinuse', {old:self.opt.nick, now: self.nick})
                self._updateMaxLineLength();
                break;
            case 'PING':
