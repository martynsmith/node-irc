...
function Client (server, nick, opt) {
  ...
  self.addListener('raw', function (message) {
    ...
    switch (message.command) {
      ...
      case '396':
      case '042': // Random welcome crap, ignoring
        break
      ...
      case 'NOTICE':
        ...
        if (self.opt.debug && to === self.nick) {
          util.log('GOT NOTICE from ' + (from ? '"' + from + '"' : 'the server') +
            ': "' + text + '"')
        }
        break
      case 'MODE':
          ...
          if (mode in self.prefixForMode) {
            ...
            if (channel.users.hasOwnProperty(modeArg)) {
              if (adding) {
                ...
                channel.users[modeArg] = channel.users[modeArg]
                                  .replace(self.prefixForMode[mode], '')
              }
            }
            ...
            chanModes(mode)
            self.emit(eventName, message.args[0], message.nick, mode,
                          undefined, message)
          }
        })
      ...
      case 'rpl_whoisidle':
        ...
        break
      case 'rpl_whoischannels': // TODO - clean this up?
        self._addWhoisData(message.args[1], 'channels', message.args[2] ? message.args[2].trim().split(/\s+/) : message.args[2])
        break
      ...
      case 'rpl_whoreply':
        ...
        self._addWhoisData(message.args[5], 'realname',
      ...
      case 'JOIN': // channel, who
        ...
        break
      case 'PART': // channel, who, reason
        self.emit('part', message.args[0], message.nick, message.args[1], message)
        self.emit('part' + message.args[0], message.nick, message.args[1], message)
        if (message.args[0] !== message.args[0].toLowerCase()) {
          self.emit('part' + message.args[0].toLowerCase(), message.nick,
        ...
        break
      case 'KICK': // channel, who, by, reason
        self.emit('kick', message.args[0], message.args[1], message.nick,
...
Client.prototype._connectionHandler = function () {
  if (this.opt.webirc.ip && this.opt.webirc.pass && this.opt.webirc.host) {
    this.send('WEBIRC', this.opt.webirc.pass, this.opt.userName, this.opt.webirc.host,
          this.opt.webirc.ip)
  }
...
Client.prototype.connect = function (retryCount, callback) {
  ...
  self.conn.setTimeout(0)
    ...
    // Each connection gets its own CyclingPingTimer. The connection forwards the timer's 'timeout'
    // and 'wantPing' events to the client object via calling the connectionTimedOut() and
    // connectionWantsPing() functions.
    //
    // Since the client's "current connection" value changes over time because of retry
    // functionality, the client should ignore timeout/wantPing events that come from
    // old connections.
...
Client.prototype._splitLongLines = function (words, maxLength, destination) {
    // If maxLength hasn't been initialized yet, prefer an arbitrarily
    // low line length over crashing.
  ...
  return this._splitLongLines(words
      .substring(cutPos + wsLength, words.length), maxLength, destination)
}
