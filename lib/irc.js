    ...
    self.addListener("raw", function (message) { // {{{
        switch ( message.command ) {
            ...
            case "NOTICE":
                ...
                if (text[0] === '\\u1' && text.lastIndexOf('\\u1') > 0) {
            ...
            case "err_umodeunknownflag":
                if ( self.opt.showErrors )
                    util.log("\\u033[01;31mERROR: " + util.inspect(message) + "\\u033[0m");
                break;
            default:
                if ( message.commandType == 'error' ) {
                    ...
                    if ( self.opt.debug )
                        util.log("\\u033[01;31mUnhandled message: " + util.inspect(message) + "\\u033[0m");
                }
                break;
        }
    }); // }}}
...
Client.prototype._handleCTCP = function(from, to, text, type, message) {
    text = text.slice(1)
    text = text.slice(0, text.indexOf('\\u1'))
