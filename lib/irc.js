...
function parseMessage(line, stripColors) { // {{{
    ...
    /* ADAPTED FROM http://calebdelnay.com/blog/2010/11/parsing-the-irc-message-format-as-a-client */
    
    var prefixEnd = -1;
    var trailingStart = line.length;
    
    var trailing;
    message.prefix = '';
    message.command = '';
    message.args = new Array();
    ...
    if (line[0] == ":")
    {
        ...
        message.prefix = line.substring(1, prefixEnd);
    }
    ...
    trailingStart = line.indexOf(" :");
    
    if (trailingStart >= 0)
        trailing = line.substring(trailingStart + 2);
    else
    ...
    var commandAndParameters = line.substring(prefixEnd + 1, trailingStart).split(' ');
    
    // The command will always be the first element of the array.
    message.command = commandAndParameters[0];
    
    // The rest of the elements are the parameters, if they exist.
    // Skip the first element because that is the command.
    if (commandAndParameters.length > 1) {
      for (var i = 1; i < commandAndParameters.length; i++) {
        message.args.push(commandAndParameters[i]);
      }
    }
