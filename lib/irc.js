    ...
    self.addListener("raw", function (message) { // {{{
        switch ( message.command ) {
            ...
            case "err_nicknameinuse":
                if (self.opt.resolveNickCollision) {
...
Client.prototype.connect = function ( retryCount, callback ) { // {{{
    ...
    self.conn.addListener("close", function() {
        ...
        self.emit('reconnect', retryCount);

        var delay = Math.min(self.opt.maxRetryDelay, self.opt.retryDelay * retryCount);
        if ( self.opt.debug ) {
            util.log( 'Waiting ' + delay + 'ms before retrying' );
        }
        setTimeout( function() {
