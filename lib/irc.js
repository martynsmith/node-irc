...
Client.prototype.send = function(command) {
    if (this.opt.floodProtection) {
        const delay = Math.max(this.opt.floodProtectionDelay, Date.now() - this.lastSendTime);
        const delayP = shouldDelayFor <= MIN_DELAY_MS ? Promise.resolve() : new Promise((r) => setTimeout(r, delay));
        const sendPromise = delayP.then(() => this._send(command));
...
Client.prototype._send = async function(command) {
