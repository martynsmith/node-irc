    ...
    self.addListener("raw", function (message) { // {{{
        switch ( message.command ) {
            ...
            case "005":
                ...
                var prefixstart = line.indexOf("PREFIX") + 7,
                    prefixes = line.substring(prefixstart, line.indexOf(" ", prefixstart)),
                    prefixmodes = prefixes.substring(1, prefixes.indexOf(")")),
                    prefixchars = prefixes.substring(prefixes.indexOf(")")+1),
                    prefixarr = {};
                for (var i = 0; i < prefixmodes.length; i++) {
                    prefixarr[prefixmodes.charAt(i)] = prefixchars.charAt(i);
                }
                if (self.opt.debug) {
                    console.log("Prefixes: ", prefixarr);
                }
                self.prefixes = prefixarr;
            ...
            case "MODE":
                ...
                if ( message.args.length >= 3 ) {
                    ...
                    for (var i = 0; i < mode.length; i++) {
                        if (adding) {
                            channel.users[message.args[nicklist_offset+i]] = self.prefixes[mode[i]];
                        }
                    }
                }
                break;
...
function parseMessage(rawLine, stripColors) { // {{{
    ...
    message.line = line; // 005 parsing much faster
