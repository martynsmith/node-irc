...
Client.prototype.join = function(channel, callback) { // {{{
	...
	for (var i = 0; i < channels.length; i++) {
		var chan = channel[i];
		// Check if it is really a channel name (could be a key)
		if (this.supported.channel.types.indexOf(chan.charAt(0)) !== -1) {
		    this.once('join' + chan, function () {
		        ...
		        if (this.opt.channels.indexOf(chan) == -1) {
		            this.opt.channels.push(chan);
		        }
    ...
    this.send.apply(this, ['JOIN'].concat(channels));
} // }}}
